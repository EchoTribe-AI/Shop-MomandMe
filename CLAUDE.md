...
- This is the **downstream client deploy** serving `shop.mommyandmecollective.com` for The Mommy & Me Collective (`everydaywithsteph` creator). Echo-Dashboard upstream is the master; this repo cherry-picks upstream commits per `docs/UPSTREAM_SYNC.md`.
- Shared framework code lands in Echo-Dashboard first, then syncs here via cherry-pick.
- Baseline tag `v1.0-pg-launch` exists on both repos.
- Client-specific files (the `branding/` directory, Mommy & Me-only tests, deploy config) live only here.
...
- **`SHOP_SUBDOMAIN=shop.mommyandmecollective.com`** must be set in this deploy's Replit Secrets — otherwise the shop subdomain routing rewrite doesn't fire on the custom domain and `/` redirects to admin login.
...
- Don't edit shared-framework files (templates, app.py, db_schema.py) without expecting them to be overwritten on the next upstream sync — those changes belong upstream in Echo-Dashboard.
